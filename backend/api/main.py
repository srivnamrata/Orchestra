...
async def guru_life_audit():
    ...
    # Gather real context from connected services
    try:
        gh_data    = await github_service.get_recent_activity() if github_service else {}
        email_data = await email_service.get_unread_summaries() if email_service else {}
    except Exception:
        gh_data = {}
        email_data = {}
    ...
    try:
        result = await param_mitra.generate_audit(context)
