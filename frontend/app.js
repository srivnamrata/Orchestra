...
async function runGuruAudit() {
    ...
    try {
        ...
        if (window.activityFeed) {
            window.activityFeed.log(`Param Mitra: ${a.summary||'Weekly insights ready.'}`, 'success', 'PARAM_MITRA');
        }
    } catch (e) {
        container.innerHTML = `<div class="run-card" style="text-align:center;padding:32px">
            ...
            <button class="intel-btn primary" onclick="runGuruAudit()"><span class="ms sm">refresh</span> Retry</button>
        </div>`;
    }
}
...
// ============================================================================
// Veda Library Integration
// ============================================================================

async function submitVeda(text) {
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            if (window.activityFeed) window.activityFeed.log(`📚 Veda: ${data.message || 'Something went wrong'}`, 'warning', 'VEDA');
        }
    } catch (e) {
        if (window.activityFeed) window.activityFeed.log('📚 Veda: ' + e.message, 'warning', 'VEDA');
    }
}
            ...
            return `
                <div class="run-card" style="display:flex;flex-direction:column;gap:10px">
                    ...
                    <div style="display:flex;gap:6px">
                        ...
                        <button class="na-btn" onclick="if(window.activityFeed)window.activityFeed.log('Veda: Updating progress for ${safe(b.title)}...','status','VEDA')">✏️ Update</button>
...
// Call fetchBooks once on load to populate the tab
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => {
